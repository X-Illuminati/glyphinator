 ...
 *     The center flag indicates whether to center the barcode on the origin.
 *     The expert_mode flag should only be used by experts.
 *
 *   data_matrix_size(bytes, expert_mode=false)
 *     Compute the overall dimensions of the barcode (including the quiet
 *     area). The value is returned as a 2-vector (e.g. [12,12]).
 *     The bytes value should be the same as what you would pass to the
 *     data_matrix module to actually generate the symbol. (That is, it should
 *     be an array generated by one of the helper functions below.)
 *
...
/*
 * data_matrix_size - Compute the overall dimensions of the barcode
 *   including the quiet area.
 *   Note: does not account for large pullback values -- normally these should
 *   be insignificant.
 *
 * bytes - data bytes to encode (same as would be passed to data_matrix func)
 *
 * expert_mode - only use this if you are an expert
 */
function data_matrix_size(bytes, expert_mode=false) =
	let(properties=(expert_mode)?
			dm_get_props_by_total_size(len(bytes)):
			dm_get_props_by_data_size(len(bytes)))
		dm_prop_dimensions(properties)+[2,2];
 ...
 * center - indicates whether to center the barcode on the origin
 * expert_mode - only use this if you are an expert
 */
	...
	vector_mode=false, center=false, expert_mode=false)
{
	...
	//Instead, it is easier to adjust each bitmap by [word.x/2, word.y/2] to
	//bring it back to its original, non-centered position and then shift
	//the entire barcode at the end by [dims.x/2, dims.y/2].
	//When you see "offs" immediately preceding a call to 2dbitmap, this is
	//its purpose.
	...
	module drawcorner1(s, size, corner)
	{
		if (2==corner)
		{
			//offs cancels the x/y centering, leaving only the z centering
			offs=center?[+2,+1,0]:[0,0,0];
			translate(offs+[size.x-6,-2,0])
				2dbitmap(
					[
		...
		{
			//offs cancels the x/y centering, leaving only the z centering
			offs=center?[-3,-2,0]:[-4,-4,0];
			translate(offs+[size.x,0,0])
				2dbitmap(
					[
						[s[1][1],s[1][2]],
						[0,s[2][0]],
	...
	module drawcorner2(s, size, corner)
	{
		...
		{
			//offs cancels the x/y centering, leaving only the z centering
			offs=center?[0.5,3.5,0]:[0,2,0];
			translate(offs+[0,-size.y,0])
				2dbitmap(
					[
						[s[0][0]],
						[s[0][1]],
	...
	module draw_codeword(bitmap, x, y, size, corner, xadj, yadj)
	{
		if (x<0) {
			if ((corner) && (-y+4>size.y)) {
				...
				//offs cancels the x/y centering, leaving only the z centering
				offs1=center?[1.5+x/2,1.5,0]:[0,0,0];
				offs2=center?[-x/2,1.5,0]:[0,0,0];
				translate(offs1+[0,y,0])
					2dbitmap(colsplit(bitmap, x), pullback=pullback,
						vector_mode=vector_mode, center=center);
				translate(offs2+[size.x+x-2,y+yadj,0])
					2dbitmap(colsplit(bitmap, -x), pullback=pullback,
						vector_mode=vector_mode, center=center);
			}
		} else {
			if (y+2>=0) {
				//offs cancels the x/y centering, leaving only the z centering
				offs1=center?[1.5,-y/2,0]:[0,0,0];
				offs2=center?[1.5,y/2+1.5,0]:[0,0,0];
				translate(offs1+[x,y,0])
					2dbitmap(rowsplit(bitmap, y), pullback=pullback,
						vector_mode=vector_mode, center=center);
				translate(offs2+[x+xadj,-size.y+2,0])
					2dbitmap(rowsplit(bitmap, -y), pullback=pullback,
						vector_mode=vector_mode, center=center);
			} else {
				//offs cancels the x/y centering, leaving only the z centering
				offs=center?[1.5,1.5,0]:[0,0,0];
				translate(offs+[x,y,0])
	...
	module data_matrix_inner(bytes, size, corner, xadj, yadj, mark=1, space=0, i=0, x=-2, y=-5, direction=0)
	{
		...
		{
			//offs cancels the x/y centering, leaving only the z centering
			offs=center?[1,1,0]:[0,0,0];
			translate(offs+[size.x-4,-size.y+2,0])
	...
	if (size != undef)
	{
		//general offset helpers for centering the barcode
		offs=center?[size.x/2,size.y/2]:[0,0];
		offs1=center?0.5:0;
		offs2=center?1:0;
		translate([2-offs.x-offs2,size.y-offs.y-offs2]) {
			//draw the data region
			data_matrix_inner(data_bytes, size, corner, xadj, yadj, mark, space);
			//draw the L finder pattern
			translate([-1+offs.x,-size.y+1+offs1])
				2dbitmap([[for (i=[0:size.x-1]) mark]], pullback=pullback,
					vector_mode=vector_mode, center=center);
			translate([-1+offs1,offs.y-size.y+2-offs1])
				2dbitmap([for (i=[0:size.y-2]) [mark]], pullback=pullback,
					vector_mode=vector_mode, center=center);
			//draw the clock track
			translate([offs.x-offs1,offs1])
				2dbitmap([[for (i=[0:size.x-2]) (i%2)?mark:space]],
					pullback=pullback, vector_mode=vector_mode,
					center=center);
			translate([size.x-2+offs1,offs.y-size.y+2-offs2])
				2dbitmap([for (i=[1:size.y-2]) [(i%2)?mark:space]],
					pullback=pullback, vector_mode=vector_mode,
					center=center);
			//draw the quiet zone
			translate([-2+offs2+offs.x,offs1-size.y])
				2dbitmap([[for (i=[0:size.x+1]) quiet_zone]],
					pullback=pullback, vector_mode=vector_mode,
					center=center);
			translate([-2+offs1,offs.y-size.y+1+offs1])
				2dbitmap([for (i=[0:size.y]) [quiet_zone]], pullback=pullback,
					vector_mode=vector_mode, center=center);
			translate([offs.x-1,1+offs1])
				2dbitmap([[for (i=[0:size.x-1]) quiet_zone]],
					pullback=pullback, vector_mode=vector_mode,
					center=center);
			translate([size.x-1+offs1,offs.y-size.y+1+offs1])
...
//example 15 - same as example 4 but with center=true
//example 16 - same as example 5 but with center=true
//example 17 - same as example 6 but with center=true
...
// Same as example 4 but with center=true
if (example==15)
	data_matrix(
		concat(
			c40_mode(),
...
// Same as example 5 but with center=true
if (example==16)
	data_matrix(dm_ascii("Wikipedia"), mark="black", center=true);

// Same as example 6 but with center=true
if (example==17)
